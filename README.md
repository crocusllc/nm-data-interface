...
```
curl -X POST "<YOUR_API_URL>/db/login" -H "Content-Type: application/json" -d '{"username": "<USER_NAME>","password": "<USER_PASSWORD>"}'
```
...
```
curl -X POST "<YOUR_API_URL>/db/create_user" \
    -H  'Authorization: Bearer <YOUR_USER_TOKEN>' \
    -H "Content-Type: application/json" \
    -d '{"username": "<NEW_USER_NAME>","password": "<NEW_USER_PASSWORD>","user_email":"<NEW_USER_EMAIL>","user_role": "<editor|admin|viewer>"}'
...
```
curl -X POST "<YOUR_API_URL>/db/delete_user" -H 'Authorization: Bearer <YOUR_USER_TOKEN>' -H "Content-Type: application/json" -d '{"id": "<USER_ID>"}'
